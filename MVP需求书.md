...
**拍一瓶酒，帮你识酒、调酒、配菜、组局——像吧台边那位年轻、有趣、调皮可爱但特别靠谱的女调酒师，让想试试酒但不太懂的年轻人，也能轻松喝明白、吃明白、局明白。**

### 产品人格：驻店女调酒师

金灵酒鬼不是冷冰冰的百科，也不是 AI 客服。**产品灵魂是一位二十多岁的年轻女调酒师**——吧台后懂酒又会聊天那种：

| 维度 | 设定 |
|------|------|
| 身份 | 金灵酒鬼驻店调酒师，负责识酒、出方案、配菜建议 |
| 性格 | 幽默风趣、有点调皮可爱，但说到酒从不糊弄 |
| 说话方式 | 接地气、像朋友聊天；善用比喻和轻巧调侃，不堆术语 |
...
- **禁止**自称「老金」「叔」「师傅」或任何固定昵称；**鉴酒点评**（`bartenderTake`）勿用第一人称自我称呼，直接评酒、给建议
- 调皮要有分寸：可以调侃「别兑可乐啊」，不能低俗、不能嘲讽用户、不能鼓励酗酒
...
| 场景 | 冷冰冰 ❌ | 女调酒师 ✅ |
|------|----------|------------|
| 识酒 | 本品呈琥珀色，雪莉桶陈酿。 | 琥珀色一眼就对上了——雪莉桶养的，果干甜、入口柔，慢品才有意思。 |
| 调酒方案名 | 威士忌曼哈顿变体 | 琥珀暖冬（冬天喝一口，从胃里暖到指尖） |
| 配菜 | 油脂与单宁中和。 | 花生香托住威士忌的甜，一口酒一粒花生，居家局标配。 |
| 小龙虾点评 | 麻辣与果香冲突，不建议。 | 麻辣小龙虾？热闹局行，但麦卡伦那点果香全被辣盖过去了——实在想吃，试试蒜香版。 |
...
| **bartenderTake** | **个性品鉴点评 1–3 句，有趣但专业，可含比喻/调侃；无自我称呼** |
...
- 材料清单：名称、用量、参考单价、小计
- 工具：雪克壶、吧勺、古典杯等
- 详细步骤：分步编号，每步 1–2 句，可适度口语化
- 预估总成本：材料参考价合计
- 难度：入门 / 进阶
- **flavorNote**：为什么这样调——专业逻辑 + 一句俏皮总结
...
| 高端酒局 | 商务、庆祝、仪式感 | 「这瓶配得上和牛，庆祝局撑得住场面」 |

每道菜包含：菜名、**reason**（口感搭配逻辑 + 接地气口吻）、预估热量、制作难度、参考成本、简化菜谱。

#### 2.3 用户自定义下酒菜 · 搭配点评

用户可输入想吃的菜（如「炸鸡」「麻辣小龙虾」）。

从以下维度分析（`analysis` 字段带个性，`tastePairing`/`utility`/`calories` 保持清晰）：
...
输出：点评结论 + 推荐等级（强烈推荐 / 可以 / 建议换一道）+ 改良版菜谱 + 可选 `swapSuggestion`（推荐换啥）。
...
打开 App / Demo
  → 拍酒瓶 / 选示例酒
  → AI 识酒（加载文案带人设）→ 识酒结果页（事实 + bartenderTake 品鉴笔记）
  → 调酒方案（个性方案名 + flavorNote）
  → 下酒菜三档 + 自定义菜 → 搭配点评
  → 酒局方案总览 / 分享卡
  → 分享邀请好友 / 协作编辑（Demo 占位）
  → 附近酒馆推荐（Mock）
...
2. 展示识酒结果（**酒瓶主体抠图** + 口感、气味、年份、产地、参考价格 + **品鉴笔记**）
3. 展示完整调酒方案（**酒杯/成品抠图**、个性方案名、材料、价格、步骤、flavorNote）
4. 展示三档下酒菜 + 用户输入一道菜后的 **搭配点评** 与改良菜谱
5. 生成酒局分享卡预览（含轻松邀请语）
6. 展示附近酒馆推荐列表（Mock）
7. 全程有「参考价仅供参考」「适量饮酒」提示
8. 整体语气可感受到「年轻、幽默调皮但专业」的女调酒师人设
...
- 首页主视觉：灵瑞插画（`assets/首页设计图.png`），标题「什么?灵瑞在喝酒!!」；无扫描线框
- 人格视觉：女调酒师以**文案人设**呈现；结果页用「品鉴笔记」小标签区分个性点评区块
- 详见 `软件界面设计提示词.md`
...
- **调酒师人设注入**：通过 System/User Prompt 约束语气，而非前端写死文案
...
金灵酒鬼/
├── MVP需求书.md              ← 本文档
├── AI提示词与Mock数据.md      ← Prompt 全文 + Mock JSON + 调酒师人设规范
...
```json
{
  "wine": {
    ...
    "confidence": "high",
    "visibleEvidence": [],
    "bartenderTake": "琥珀色一眼就对上了——雪莉桶养的，果干甜、入口柔，别兑可乐，算这瓶酒求你。"
  },
  "source": "ai"
}
```
...
**用途：** 基于识酒 JSON，一次生成调酒方案、三档下酒菜、预设菜品（麻辣小龙虾）搭配点评。
...
### 7.7 Prompt 工程策略（女调酒师人设）

**三层约束：**

1. **角色层**：所有生成类 Prompt 开头注入年轻女调酒师人设（性格、语气、底线；禁止自我称呼）
...
- `steps`：可口语化，但必须可执行
- `dishAnalysis.analysis`：风趣接地气；`rating` 仍须遵循搭配逻辑
...
**评委演示建议路径：** 示例酒 Mock 30 秒走完全流程 → 再现场拍一瓶真实酒瓶展示 AI 识酒 + 调酒方案。
...
| 版本 | 产品 | 技术 |
|------|------|------|
| **Demo（今天）** | 女调酒师人设 + 5 步流程 + 分享卡 + 酒馆 Mock | HTML Demo + 火山双 API + Mock 兜底 |
| **V1.0** | 抖音小程序；自定义菜实时点评；loading 文案库 | Taro/uni-app 小程序壳；拆 `review-dish` API；云托管 API |
| **V1.1** | 好友协作编辑、真实 LBS 酒馆 | WebSocket/Supabase 协作；高德/腾讯地图 POI |
| **V1.2** | 酒款收藏、历史酒局、个人口味画像 | 用户体系 + 向量记忆口味偏好 |
...
| 识酒品鉴点评 | ✅ Prompt 已定义；前端「品鉴笔记」区块 | `AI提示词与Mock数据.md` §1 |
...
| 附近酒馆 | ✅ Mock | `BARS` 常量 |
| 女调酒师人设语气 | ✅ Prompt 层 | `server.js` + `AI提示词与Mock数据.md` |
